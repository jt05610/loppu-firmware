...
static struct
{
    ...
    volatile uint16_t dma_pos_old;
    volatile uint16_t dma_pos_new;
    volatile uint16_t rx_pos_old;
#if STM32_USART_DMA_BUFFER_SIZE
    uint8_t  dma_buffer[STM32_USART_DMA_BUFFER_SIZE];
    rx_complete_callback_t rx_callback;
#endif
...
close(void * instance)
{
    ...
    LL_DMA_DisableChannel(DMA1, TX_CHANNEL);
}
