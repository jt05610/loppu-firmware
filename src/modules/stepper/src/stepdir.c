...
stepdir_rotate(StepDir base, int32_t vel) {
    ...
    if (vel > 0) {
        tmc_ramp_linear_set_targetVelocity(
            base->ramp, MIN(STEPDIR_MAX_VELOCITY, vel));
    }
    if (vel < 0) {
        tmc_ramp_linear_set_targetVelocity(
            base->ramp, MAX(vel, -STEPDIR_MAX_VELOCITY));
    }
}
...
stepdir_periodic_job() {
    ...
    if (dx) {
        if (dx <= 0) {
            stepper_set_dir(self.stepper, 0);
        } else {
            stepper_set_dir(self.stepper, 1);
        }
        gpio_set_pin(
            self.stepper->hal->gpio, self.stepper->port,
            self.stepper->step_pin);
...
stepdir_get_status(StepDir base) {
    ...
    status |= (stepdir_get_target_pos(base) == stepdir_get_pos(base)) ? STEPDIR_STATUS_TARGET_REACHED : 0;
...
stepdir_set_target_vel(StepDir base, int32_t vel) {
    if (vel > 0) {
        ...
        tmc_ramp_linear_set_acceleration(
            base->ramp, abs(tmc_ramp_linear_get_acceleration(base->ramp)));
    }
    if (vel < 0) {
        tmc_ramp_linear_set_targetVelocity(
            base->ramp, MAX(vel, -STEPDIR_MAX_VELOCITY));
        tmc_ramp_linear_set_maxVelocity(
            base->ramp, MAX(vel, -STEPDIR_MAX_VELOCITY));
        tmc_ramp_linear_set_acceleration(
            base->ramp, -1 * abs(tmc_ramp_linear_get_acceleration(base->ramp)));
    }
}
