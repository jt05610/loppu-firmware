...
axis_nudge(Axis axis, int32_t amount) {
    stepdir_move_rel(_SD, amount * axis->steps_per_mm * (1 << _SD->ms) / 1000);
}
...
void axis_set_steps_per_mm(Axis base, int32_t steps_per_mm) {
    base->steps_per_mm = steps_per_mm;
}

int32_t axis_get_steps_per_mm(Axis base) {
    return base->steps_per_mm;
}